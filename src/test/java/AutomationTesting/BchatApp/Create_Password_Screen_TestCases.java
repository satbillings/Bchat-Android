...
public class Create_Password_Screen_TestCases extends baseclass {
	...
	/*
	  Validate the next function with a value in Enter password and without a value in Re-Enter password.
	 */
	
	  @Test(priority = 4) 
	  public void To_Validate_the_next_function_with_value_only_In_Enter_password_Field () { 
	  createpasswordpage =new CreatePasswordScreen(driver);
	  Assert.assertEquals(createpasswordpage.pageTitle(),"Create Password");
	  Assert.assertEquals(createpasswordpage.textEnter(),"Enter your PIN");
	  createpasswordpage.setPassword_0(); 
	  ...
	  /* Validate the Pin number fields in the Create password screen by entering values below boundary value in any one field */
	  
	  @Test(priority =5) 
	  public void To_Validate_the_Password_number_fields_by_entering_values_below_boundary_value_in_any_one_field (){
	  createpasswordpage =new CreatePasswordScreen(driver);
	  Assert.assertEquals(createpasswordpage.pageTitle(),"Create Password");
	  ...
	  /* Validate whether the value entered in the Pin number fields of the Create password is editable. 
	     Validate whether the value entered in the Pin number fields of the Create password is delete-able. */
	  
	  @Test(priority=6) 
	  public void To_validate_Whether_Values_In_Password_Fields_are_Editable_And_deletable () {
	  createpasswordpage =new CreatePasswordScreen(driver);
	  Assert.assertEquals(createpasswordpage.pageTitle(),"Create Password");
	  Assert.assertEquals(createpasswordpage.textEnter(),"Enter your PIN");
	  createpasswordpage.setPassword_0();
	  ...
	  /*Validate with Valid value in enter password field and Invalid value  in Re-enter password field and repeat it for multiple times. */
	  
	  @Test(priority = 7) 
	  public void To_Validate_with_Valid_value_in_enter_password_field_and_Invalid_value_in_ReEnter_password_field_and_repeat_it_for_multiple_time(){
	  for(int i=0;i<3;i++) { 
	  createpasswordpage =new CreatePasswordScreen(driver);
	  Assert.assertEquals(createpasswordpage.pageTitle(),"Create Password");
	  Assert.assertEquals(createpasswordpage.textEnter(),"Enter your PIN");
	  ...
	  /* Validate with Valid value in enter password field and Valid value in Re-enter password field. */
	  
	  @Test(priority = 8) 
	  public void To_Validate_with_Valid_value_in_both_enter_password_field_and_in_ReEnter_password_field () {
	  createpasswordpage =new CreatePasswordScreen(driver);
	  Assert.assertEquals(createpasswordpage.pageTitle(),"Create Password");
	  Assert.assertEquals(createpasswordpage.textEnter(),"Enter your PIN");
